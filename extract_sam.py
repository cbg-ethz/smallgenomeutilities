...
import argparse
from __mapper_impl__ import convert_from_intervals_to_list, find_interval_on_dest
import pysam
import os
import sys
import Bio.Seq
...
parser.add_argument("-o", dest="OUTPUT", help="Output FASTA file", default="", metavar="output")
...
for contig in samfile.references:
	result = convert_from_intervals_to_list(find_interval_on_dest(contig, TO_CONTIG, COORDINATES, 0, MSA_FILE, VERBOSE))
	contig_loci_map[contig] = contig_tuple(map=result, locus_start=result[0], locus_end=result[-1])
...
for record in samfile.fetch():
	...
	if read_id in reads:
		...
		if reads[read_id][1].reference_start < reads[read_id][0].reference_start:
			reads[read_id][0], reads[read_id][1] = reads[read_id][1], reads[read_id][0]
	else:
...
def seq_builder(have_already_loci, subsequence, read, contig_details):
	...
	if (contig_details.locus_start in range(read.reference_start, read.reference_end)) or (
...
for identifiers, mates in reads.items():
	...
	if len(subsequence) < len(contig_tuple.map) and len(mates) > 1:
		...
		subsequence, new_loci = seq_builder(loci_covered, subsequence, mates[1], contig_tuple)
